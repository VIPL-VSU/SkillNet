...
@dataclasses.dataclass(frozen=True)
class TokenizeSubtaskScheme1Training(DataTransformFn):
    """Tokenize mixed scheme1 subtask samples.

    Expected input key:
      subtask_training_target: "subtask_ce" or "action_fm".

    subtask_ce samples train subtask generation with CE. action_fm samples
    train action flow with the full subtask as non-causal text context.
    """
    tokenizer: _tokenizer.PaligemmaTokenizer
    ...
    def __call__(self, data: DataDict) -> DataDict:
        prompt = data.pop("prompt", None)
        if prompt is None:
            raise ValueError("Prompt is required for scheme1 subtask training")
        if not isinstance(prompt, str):
            prompt = prompt.item()
        ...
        target = data.pop("subtask_training_target", None)
        if target is None:
            raise ValueError("subtask_training_target is required for scheme1 subtask training")
        ...
        if target == "subtask_ce":
            tokens, mask, ar_mask, loss_mask = self.tokenizer.tokenize_high_low_prompt(high_prompt, low_prompt)
            target_id = 0
        elif target == "action_fm":
            tokens, mask, ar_mask, loss_mask = self.tokenizer.tokenize_high_low_prompt_condition(
                high_prompt, low_prompt
            )
            target_id = 1
        else:
            raise ValueError(f"Unknown subtask_training_target: {target!r}")
        ...
        return {
            **data,
            ...
            "token_ar_mask": ar_mask,
            "token_loss_mask": loss_mask,
            "subtask_training_target_id": np.asarray(target_id, dtype=np.int32),
        }
