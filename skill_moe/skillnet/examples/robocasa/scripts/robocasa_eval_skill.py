...
groot_path = Path(os.environ.get("GROOT_PATH", REPO_ROOT / "examples" / "robocasa" / "gr00t"))
...
from openpi_client import websocket_client_policy_robocasa as _websocket_client_policy
...
class Args:
    ...
    video_dir: str = "data/robocasa/videos"
    n_episodes: int = 50
...
def prompt2skill(prompt):
    ...
    return repo_list[prompt.split(' ')[0].lower()]
...
def eval_robocasa(args: Args) -> None:
    ...
    for i in trange(args.n_episodes):
        ...
        while not done:
            ...
            # state : shape=(53,)
            # prompt : str
            # print(obs["annotation.human.action.task_description"][0])
            element = {
                ...
                "skills": str(prompt2skill(obs["annotation.human.action.task_description"][0]))
            }
            # print(element)
